...
if __name__ == '__main__':
    ...
    nomal_size = 160 # 普通字体大小 200
    right_distance = 400 # 右侧到图片边缘的距离
    rd = right_distance
    data = "2020 年 11 月" # 图片中显示的时间
    fbt_image_height = 3500 # 插入的分布图的高度 4400
    fbt_name_y = 2600 # 1620
    ...
    for bzp_data in dataf.itertuples():
        ...
        ad.add_text_column(texts1, (350,1990,0,560), charactor_font, nomal_size)
        # ad.add_text_column(value1_1, (2550,1990,0,560), charactor_font, nomal_size)
        ad.add_text_column(value1_1, (1950,1990,0,560), charactor_font, nomal_size)
        # 标题
        XJQYMC = getattr(bzp_data, "乡镇名称")
        title = text_space(city_name +XJQYMC)
        print("标题：",title)
        title = "{} 粮 食 生 产 功 能 区".format(title)
        ad.text_center_y(title
                         , 718, charactor_font_bd, 300, color="red")
        # 绘制标志牌
        ad.add_text_column("标 志 牌 编 号：", (462, 357, 0, 0), charactor_font_bd, 100)
        ad.add_text_column(str(getattr(bzp_data,"标志牌编号")), (1200, 357, 0, 0), charactor_font_bd, 100)
        
        # 绘制右下角文字
        # 金 堂 县 三 星 镇 人 民 政 府   立
        gov=text_space("{} 立".format(value1[-2]))
        ...
        try: # 如果没有找到和表格数据对应的图片会报错
            ...
            print(e.strerror)
            continue
        x_topleft, y_topleft =9550-fbt_w, 6400-fbt_h  # 9400
        x_bottomright, y_bottomright = 9550, 6400
        # 粘贴区域大小
